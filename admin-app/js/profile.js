...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchSessions() {
    ...
    try {
      ...
      const currentToken = localStorage.getItem("rswallet_admin_session_token");
  ...
  async function fetchProfile() {
    try {
      ...
      if (data && data.length > 0) {
        ...
        profileDisplayName.textContent = adminData.admin_name || "RsWallet Admin";
      } else {
        // No admin settings found, use defaults
        profileName.value = "RsWallet Admin";
        profileEmail.value = "admin@rswallet.com";
      }
    } catch (e) {
  ...
  saveProfileBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (newSessionToken) {
        localStorage.setItem("rswallet_admin_session_token", newSessionToken);
      }
