...
export async function GET() {
  const [slides, banners, video, telegram] = await Promise.all([
    supabaseRest('slider_images?select=*&is_enabled=eq.true&order=display_order.asc', { method: 'GET' }).catch(() => []),
    supabaseRest('banners?select=*&is_enabled=eq.true&order=display_order.asc', { method: 'GET' }).catch(() => []),
  ...
  return NextResponse.json({ slides, banners, video: video?.[0] || null, telegram: telegram?.[0] || null });
}
