...
export async function GET(request: Request) {
  ...
  try { const [content, assets] = await Promise.all([readContent(), readAssets()]); return json({ content, assets, localPreview: (await adminIdentity(request.headers)).localPreview }); }
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return json({ content: await readContent(), assets, localPreview: (await adminIdentity(request.headers)).localPreview });
