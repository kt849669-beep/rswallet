...
export default function AdminDashboard() {
  ...
  if (!data) return <main className="center-screen">Loading RS Wallet admin…</main>;
  return (
    <main className="admin-shell">
      <aside>
        <div className="admin-brand"><span>R</span><div><h1>RS Wallet</h1><small>ADMIN PANEL</small></div></div>
      ...
      <section>
        <div className="section-heading"><div><p>RS Wallet control centre</p><h2>{label(tab)}</h2></div><button className="outline-button" onClick={refresh}>Refresh</button></div>
...
function Media({ resource, rows, save, remove }) {
  ...
  return <>
    ...
    {!rows.length && <div className="empty-state">No images yet. Upload the first RS Wallet {resource === 'sliders' ? 'home slide' : 'reward banner'} above.</div>}
  </>;
}
