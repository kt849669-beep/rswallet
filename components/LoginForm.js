...
export default function LoginForm() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (data.limited) return setLimited(data.telegramUrl || 'https://app-web.showpay-web.com');
      sessionStorage.setItem('showpay_session', JSON.stringify(data.session));
      router.push('/home');
    } catch (cause) {
  ...
  return (
    <>
      <form className="login-form" onSubmit={submit}>
        ...
        <div className="forgot"><a href="https://app-web.showpay-web.com/forgetPassword">Forget Password</a></div>
        {error && <p className="form-error" role="alert">{error}</p>}
        <button className="primary login-button" disabled={loading}>{loading ? 'Please wait...' : 'LOG IN'}</button>
        <p className="register">Don&apos;t have an account? <a href="https://app-web.showpay-web.com/regist?code=2invite5p6">Register</a></p>
      </form>
