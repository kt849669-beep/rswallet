...
function NavIcon({ name }) {
  ...
  if (name === 'upi') return <svg viewBox="0 0 32 32" aria-hidden="true"><path d="M5 10h22v17H5zM9 5h14l2 5H7l2-5Z" /><path d="M11 15h10M16 12v11" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" /></svg>;
...
export default function UserHome() {
  ...
  return (
    <main className="phone-shell">
      ...
      {stage === 'mpin' && <div className="modal mpin-modal" role="dialog" aria-modal="true" aria-labelledby="mpin-title"><div className="dialog mpin-dialog"><button className="dialog-close" type="button" onClick={logout} aria-label="Close and return to login">×</button><div className="dialog-brand">RsWallet</div><h2 id="mpin-title">Enter Your MPIN</h2><p>Enter your secure 6-digit MPIN to continue</p><div className="mpin" onPaste={pasteMpin}>{digits.map((value, index) => <input key={index} ref={(node) => { inputs.current[index] = node; }} value={value} onChange={(event) => changeDigit(index, event.target.value)} onKeyDown={(event) => { if (event.key === 'Backspace' && !value && index > 0) inputs.current[index - 1]?.focus(); }} inputMode="numeric" pattern="[0-9]*" autoComplete={index === 0 ? 'one-time-code' : 'off'} maxLength={1} aria-label={`MPIN digit ${index + 1}`} disabled={savingMpin} autoFocus={index === 0} />)}</div>{mpinError && <p className="mpin-error" role="alert">{mpinError}</p>}<span className="mpin-hint">{savingMpin ? 'VERIFYING…' : 'MPIN will submit automatically'}</span></div></div>}
