...
export default function AdminLogin() {
  ...
  return <main className="admin-login"><form onSubmit={submit}><div className="shield">◆</div><h1>Admin Portal</h1><p>Secure access to ShowPay controls</p><label>Email Address<input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} required /></label><label>Password<input type="password" value={password} onChange={(e)=>setPassword(e.target.value)} required /></label>{error&&<p className="form-error">{error}</p>}<button className="primary">Authenticate</button></form></main>;
}
